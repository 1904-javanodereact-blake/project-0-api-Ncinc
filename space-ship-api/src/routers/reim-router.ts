...
reimRouter.get('/status/:statusId', [ authMiddleware(['admin', 'finance']), async (req, res) => {
  ...
  reimRouter.patch('/update',  [ authMiddleware(['admin']),
    async(req, res) => {
      const { reimbursementlistid } = req.body;
    ...
    prevRecord.amount, prevRecord.date_submitted, prevRecord.date_resolved, prevRecord.description,
     prevRecord.resolver, prevRecord.status, prevRecord.type);
